...
from part1.AIMakeup import Makeup,Face,Organ,NoFace

def menubar_add_face_ai(instance):
    instance.main_window_face_ai = QMainWindow()
    instance.win_face_ai = Ui_MainWindow(instance.main_window_face_ai,instance.root_path,instance)
    #instance.win_face_ai = Ui_MainWindow(instance.main_window_face_ai, instance.root_path)
    face_ai_action = QAction('&打开窗口', instance)
    face_ai_action.setObjectName("face_ai_action")
    face_ai_action.triggered.connect(instance.show_face_ai_show)

    menubar = instance.menuBar()
    face_ai_Menu = menubar.addMenu('&人脸美化')
    face_ai_Menu.addAction(face_ai_action)


class Ui_MainWindow(object):
    def __init__(self, MainWindow,root_path,parrent_object):
        self.parrent=parrent_object
        ...
        # 导入dlib模型文件
        if os.path.exists(self.root_path+"/part1/shape_predictor_68_face_landmarks.dat"):
            self.path_predictor = os.path.abspath(self.root_path+"/part1/shape_predictor_68_face_landmarks.dat")
        else:
            ...
            self.path_predictor, _ = QFileDialog.getOpenFileName(self.centralWidget, '选择dlib模型文件', './',
                                                                 'Data Files(*.dat)')
        # 实例化化妆器
    ...
    def _set_connect(self):
        ...
        for op in ['sharpen', 'whitening', 'smooth', 'brightening', 'cancel', 'confirm', 'reset', 'save',
                   'save_compare', 'view_compare']:
    ...
    def _read_img(self, im_rgb):

        self.im_bgr = cv2.cvtColor(im_rgb, cv2.COLOR_RGB2BGR)
        im_hsv = cv2.cvtColor(self.im_bgr, cv2.COLOR_BGR2HSV)
        self.temp_bgr, temp_hsv = self.im_bgr.copy(), im_hsv.copy()
        self.faces = self.mu.get_faces(self.im_bgr, im_hsv, self.temp_bgr, temp_hsv, "image_in_face")

        self.path_img="image_in_face"#TODO:
    ...
    def _save(self):
        self.parrent.m_image = cv2.cvtColor(self.temp_bgr, cv2.COLOR_BGR2RGB)
        self.parrent.updata_image()

        output_path, _ = QFileDialog.getSaveFileName(self.centralWidget, '选择保存位置', './',
                                                     'Image Files(*.png *.jpg *.bmp)')
        if output_path:
            #self.save(output_path, self.im_bgr)
            #self.parrent.m_image=cv2.cvtColor(self.im_bgr,cv2.COLOR_BGR2RGB)
