...
class Ui_MainWindow(object):
    def __init__(self, MainWindow):
        ...
        #导入dlib模型文件
        if os.path.exists("F:/AIMakeup-master/shape_predictor_68_face_landmarks.dat"):
            self.path_predictor=os.path.abspath("F:/AIMakeup-master/shape_predictor_68_face_landmarks.dat")
        else:
...
class main_widget(QMainWindow):
    def __init__(self):
        super().__init__()
        bt=QtWidgets.QPushButton(self)


        self.show()
        self.aaa=QtWidgets.QMainWindow()
        #self.widd=QtWidgets.QWidget()
        self.t=Ui_MainWindow(self.aaa)
        bt.clicked.connect(self.showw)
    def showw(self):
        self.aaa.show()
